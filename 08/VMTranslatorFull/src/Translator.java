...
public class Translator {
    ...
    public List<String> translate(List<String> content) {
        ...
        for(String command : content) {
            ...
            if(first.equals("push")) {
                ...
                commandResolver.Label(second);
            } else if(first.equals("goto")) {
                commandResolver.Goto(second);
            } else if(first.equals("if-goto")) {
                commandResolver.IfGoto(second);
            } else if(first.equals("call")) {
                commandResolver.Call(second, third);
            } else if(first.equals("function")) {
                commandResolver.Function(second, third);
            } else if(first.equals("return")) {
                commandResolver.Return();
            } else if(first.equals("NEW_FILE")) {
                commandResolver.NewFile(second);
            } else {
    ...
    private String withoutComment(String command) {
        if(command.contains("//") && command.split("//").length > 0) {
